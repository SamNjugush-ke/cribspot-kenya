...
const safeEditorData = (raw: any): OutputData => {
  if (!raw || typeof raw !== "object") return { time: Date.now(), blocks: [] };
  return {
...
function BlogEditorInner() {
  ...
  const editorDataRef = useRef<OutputData>({ time: Date.now(), blocks: [] });
  ...
  const normalizeEditorImageUrls = (data: OutputData): OutputData => {
    ...
    return { ...safeEditorData(data), blocks };
  };
  ...
  class MediaLibraryImageTool {
    ...
    static get toolbox() {
      return {
        ...
        icon: `<svg width="18" height="18" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M4 5h16v14H4V5zm2 2v10h12V7H6zm2 7l2-2 2 2 3-4 3 4v2H8v-2z" fill="currentColor"/></svg>`,
      };
    }
    ...
    render() {
      ...
      this.wrapper.innerHTML = `<div style="padding:10px;border:1px dashed #ddd;border-radius:10px;color:#666;font-size:13px;">Select an image from Media Library…</div>`;
      return this.wrapper;
    }
    ...
    try {
      ...
      const tools: any = {
        ...
        mediaLibraryImage: {
          ...
          config: { open: () => setInlinePickerOpen(true) },
        },
      };

      const normalized = normalizeEditorImageUrls(data);
      editorDataRef.current = normalized;
        ...
        onChange: async () => {
          ...
          try {
            if (editorRef.current) {
              const snapshot = await editorRef.current.save();
              editorDataRef.current = normalizeEditorImageUrls(snapshot as OutputData);
            }
          } catch {
      ...
      const message = e?.message || "Failed to initialize editor";
      setErr(message);
      toast.error(message);
      editorRef.current = null;
    } finally {
  ...
  const buildPayload = async () => {
    let data = editorDataRef.current;

    if (editorRef.current) {
      const raw = await editorRef.current.save();
      data = normalizeEditorImageUrls(raw as OutputData);
    ...
    const load = async () => {
      ...
      try {
        ...
        if (id) {
          ...
          const nextEditorData = safeEditorData((b as any).contentJson);
          editorDataRef.current = normalizeEditorImageUrls(nextEditorData);
          setPendingEditorData(editorDataRef.current);
          ...
          const emptyData = { time: Date.now(), blocks: [] };
          editorDataRef.current = emptyData;
          setPendingEditorData(emptyData);
        }
      } catch (e: any) {
  ...
  useEffect(() => {
    if (loading || !pendingEditorData) return;
    void initEditor(pendingEditorData);
  }, [pendingEditorData, loading]);
  ...
  const setPublishState = async (next: boolean) => {
    ...
    if (next && !coverImage.trim()) {
      const message = "Add a cover image before publishing this post.";
    ...
    try {
      ...
      setPublished((prev) => (next ? false : prev));
      ...
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Button onClick={save} disabled={savingManual || initingEditor || slugStatus === "taken"}>
            <Save className="mr-2 h-4 w-4" /> {savingManual ? "Saving…" : "Save"}
          </Button>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="border rounded-xl bg-white p-4 lg:col-span-2">
          <div className="space-y-3">
            ...
            <div>
              <div className="mb-1 text-xs font-semibold text-gray-600">Slug</div>
              <Input
                value={slug}
                onChange={(e) => {
                  setSlugTouched(true);
            ...
            <div>
              <div className="mb-1 text-xs font-semibold text-gray-600">Excerpt</div>
            ...
            <div className="flex items-center justify-between">
              ...
              <Button variant="outline" size="sm" onClick={() => setInlinePickerOpen(true)} disabled={!editorRef.current}>
                <ImageIcon className="mr-2 h-4 w-4" /> Insert from Media
              </Button>
            </div>

            <div className="border rounded-lg">
              <div id={editorHolderId} className="min-h-[320px] p-3" />
            </div>
        ...
        <div className="space-y-6 border rounded-xl bg-white p-4">
          <div>
            <div className="flex items-center justify-between">
            ...
            {coverImage ? (
              <img src={resolveUrl(coverImage)} alt="Cover" className="mt-2 h-40 w-full rounded-lg object-cover" />
            ) : (
              <div className="mt-2 text-xs text-gray-500">No cover selected.</div>
            )}

            {publishError && <div className="mt-2 text-xs text-red-700">{publishError}</div>}
          </div>
          ...
          <div className="rounded-xl border p-3">
            <div className="flex items-start justify-between gap-3">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant={published ? "outline" : "default"} disabled={!id || !canPublish || !!deletedAt} onClick={() => setPublishState(true)}>
          ...
          <div className="space-y-2">
            ...
            <div>
              <div className="mb-1 text-xs text-gray-600">SEO Title</div>
              <Input value={seoTitle} onChange={(e) => setSeoTitle(e.target.value)} placeholder="Optional (defaults to title)" />
            </div>

            <div>
              <div className="mb-1 text-xs text-gray-600">SEO Description</div>
              <Textarea
                value={seoDesc}
            ...
            <div>
              <div className="mb-1 text-xs text-gray-600">SEO Keywords</div>
              <div className="mb-2 flex flex-wrap gap-2">
                {seoKeywordsArr.length === 0 && <div className="text-xs text-gray-500">Add keywords like a CMS: type and press Enter.</div>}
                {seoKeywordsArr.map((k) => (
                  <span key={k} className="inline-flex items-center gap-2 rounded-full border bg-white px-3 py-1 text-xs">
                    {k}
                    <button type="button" onClick={() => removeKeyword(k)} className="text-gray-500 hover:text-gray-900" aria-label="Remove keyword">
                      ×
                    </button>
                  </span>
                ))}
