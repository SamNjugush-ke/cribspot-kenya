...
/**
 * Impersonation storage convention (frontend-only):
 * - rk_token = current session token (maybe impersonated)
 * - rk_token_impersonator = the original admin/super token (only set while impersonating)
 * - rk_impersonated_user = optional JSON { id, email, name, role }
 * - rk_impersonator_user = optional JSON { id, email, name, role }
 *
 * Your existing impersonation flow can keep doing whatever it does,
 * as long as it sets rk_token_impersonator before swapping rk_token.
 */
...
export default function DashboardHeader({
  ...
  const impersonatedUser = useMemo(
    () => safeJson<{ id: string; email?: string; name?: string; role?: string }>(typeof window !== "undefined" ? localStorage.getItem("rk_impersonated_user") : null),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [pathname]
  );
  const impersonatorUser = useMemo(
    () => safeJson<{ id: string; email?: string; name?: string; role?: string }>(typeof window !== "undefined" ? localStorage.getItem("rk_impersonator_user") : null),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [pathname]
  );
  ...
  // Lightweight “global” search UX:
  // sends you to Users/Listings with q as query param (optional to implement there)
  const quickSearch = (e: React.FormEvent) => {
    e.preventDefault();
    const needle = q.trim();
    if (!needle) return;

    // If you prefer a dedicated /dashboard/super/search later, route there instead
  ...
  const logout = () => {
    ...
    // Clear both tokens + any impersonation markers
  ...
  const revertImpersonation = () => {
    ...
    localStorage.removeItem("rk_token_impersonator");
    localStorage.removeItem("rk_impersonated_user");
    // keep impersonator_user optional (but safe to remove)
    localStorage.removeItem("rk_impersonator_user");

    // Hard refresh the app state
  ...
  return (
    <div className={cn("sticky top-0 z-40", className)}>
      {/* Impersonation bar (always visible while impersonating) */}
      {isImpersonating && (
        <div className="w-full border-b border-red-300 bg-red-50">
          <div className="mx-auto max-w-[1400px] px-4 py-2 flex flex-wrap items-center gap-2 justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-xs text-gray-600 truncate">
                    {subtitle ?? "Shortcuts, search, and account controls — all in one place."}
                  </div>
                </div>
              </div>
            </div>
              ...
              <Card className="shadow-soft rounded-2xl border">
                <div className="flex items-center gap-2 px-3 py-2">
                  ...
                  <Button
                    type="submit"
                    size="sm"
                    className="bg-brand-blue text-white hover:bg-brand-sky"
                  >
                    Go
                  </Button>
                </div>
            ...
            <div className="flex items-center gap-2 justify-between lg:justify-end">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-[320px]">
                  <DropdownMenuLabel className="space-y-1">
                    ...
                    {isImpersonating ? (
                      <div className="text-xs text-red-700 mt-2">
                        You’re currently impersonating. Revert when done.
                      </div>
                    ) : null}
                  </DropdownMenuLabel>
                  ...
                  <DropdownMenuItem asChild>
                    <Link href="/dashboard/super" className="cursor-pointer">
